//Serial TAP connection to a paging base
...
//set port number
port = config.ports.tapOutPort;
//set baud rate etc here, defaults to 9600, 8, 1, none, if you need flow control check SerialPort docs
baud = parseInt(config.ports.tapOutBaud);
databits = parseInt(config.ports.tapOutDataBits);
stopbits = parseInt(config.ports.tapOutStopBits);
Parity = config.ports.tapOutParity;
...
//send page to page base, will retry up to 5 times
